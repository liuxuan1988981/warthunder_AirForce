...
# 解析json文件
def paras_json(json_file, class_name_file, dResult):
    ...
    for shape in shapes:
        ...
        if class_name in dResult:
            dResult[class_name] += 1
        else:
            dResult[class_name] = 1
        if class_id < 0:
            continue
...
def main(class_name='train.names', img_dir='JPEGImages', train_txt='train.txt', labels_dir='labels'):
    ...
    for f in os.listdir(img_dir): 
        ...
        for img_file in curr_dir_imgs:
            ...
            if json_file:
                ...
                # 得到json信息 list
                json_inf = paras_json(json_file, class_name, dClass)
                # 标签文件名
    ...
    print("\n 保存训练图片路径到: ", train_txt)
    print(json.dumps(dClass, indent=4))
