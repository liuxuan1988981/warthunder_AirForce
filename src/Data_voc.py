...
class Data():
    def __init__(self, voc_root_dir, voc_names_file, class_num, batch_size, anchors, width, height, is_tiny=False):
        ''' 
        is_tiny:the flag that loading the yolo_tiny's data
        '''
        Log.add_log("message:voc_root_dir '"+str(voc_root_dir)+"', batch_size '"+str(batch_size)+"', size '"+str(width)+","+str(height)+"'")
    ...
    def __init_args(self):
        ...
        if self.anchors is None:
            self.anchors = tools.get_anchors(self.labels_path, self.imgs_path, target_size=[self.width, self.height], k=9)
        self.anchors = np.asarray(self.anchors).astype(np.float32).reshape([-1, 2]) / [self.width, self.height]
        Log.add_log("message:Data:anchors_ori:"+str(self.anchors * [self.width, self.height]))
